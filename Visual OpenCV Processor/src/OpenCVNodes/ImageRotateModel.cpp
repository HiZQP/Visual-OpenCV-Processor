...
ImageRotateModel::ImageRotateModel()
{
	...
	connect(_angle, &QDoubleSpinBox::editingFinished, this, &ImageRotateModel::calculate);
}
...
void ImageRotateModel::setInData(std::shared_ptr<QtNodes::NodeData> nodeData, QtNodes::PortIndex portIndex)
{
	if (nodeData) {
		if (portIndex == 0) {
			...
			_angle->setValue(static_cast<int>(numberData->get()));
