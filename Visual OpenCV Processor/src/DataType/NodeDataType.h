...
public:
	PointsData() : _points() {}
	PointsData(const std::vector<cv::Point2d>& points) : _points(points) {}
	QtNodes::NodeDataType type() const override { return QtNodes::NodeDataType{ "Points", "点集" }; }
	std::vector<cv::Point2d> get() const { return _points; }
};

class LineData : public QtNodes::NodeData
{
private:
	cv::Vec4i _line;
public:
	LineData() : _line(cv::Vec4i(0, 0, 0, 0)) {}
	LineData(const cv::Vec4i& line) :
		_line(line) {}
	QtNodes::NodeDataType type() const override { return QtNodes::NodeDataType{ "Line", "线段" }; }
